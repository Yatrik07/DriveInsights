...
class DBOperations:
    ...
    def create_Database_Table(self):
        ...
        """
        # self.logger.write_logs("Entered function create_Database_Table.")
        conn_prediction = sqlite3.connect('database//predictionDatabase.db')
        # self.logger.write_logs("Created / Connected prediction Database successfully!")
        cursor_obj = conn_prediction.cursor()
        cursor_obj.execute('''CREATE TABLE if not exists Predictions(Time VARCHAR(25), Number_Plate VARCHAR(15) , user_name VARCHAR(25), Driver_name VARCHAR(25) , Vehicle_type VARCHAR(15) , phone_number VARCHAR(12) , Destination VARCHAR(10) , Puspose VARCHAR(50))'''); # Sr INTEGER AUTOINCREMENT,

        conn_user = sqlite3.connect('database//users.db')
        cursor_obj_user = conn_user.cursor()

        cursor_obj_user.execute('''CREATE TABLE if not exists user_details(user_name VARCHAR(25) , password VARCHAR(25))'''); # Sr INTEGER AUTOINCREMENT,
        # self.logger.write_logs("Train Table created successfully!")
        conn_prediction.close()
        conn_user.close()


    def enter_recordTo_Table(self ,number_plate = None , user_name = None , password = None , type = None, Driver_name =None , Vehicle_type = None , phone_number = None, Destination = None,  Puspose = None):
        # cement, blast_furnace_slag, fly_ash, water, superplasticizer, coarse_aggregate, fine_aggregate, age
        """
        Method Name : enter_recordTo_Table
        ...
        if type == "Predictions":
            ...
            cursor_obj.execute(
                "insert into Predictions(Time, Number_Plate , user_name, Driver_name, Vehicle_type, phone_number, Destination,  Puspose ) values(?,?,?,?,?,?,?,?)",
                [str(current_date_time), number_plate , user_name , Driver_name, Vehicle_type, phone_number, Destination,  Puspose ])
            conn.commit()
            conn.close()
        ...
        if type == "user_details":
            ...
            cursor_obj.execute(
                "insert into user_details(user_name, password) values(?,? )",
                [str(user_name), password])
            conn.commit()
            print("successfully entered username pswd to table")
            conn.close()

    def showTable(self, table_name , user_name = None):
        """
        Method Name : showTable
        ...
        if table_name == "Predictions":
            ...
            data = cursor_obj.execute('''SELECT * FROM Predictions where user_name = ?''' , [user_name])

        if table_name == "user_details":
            conn = sqlite3.connect('database//users.db')
            cursor_obj = conn.cursor()
            output = cursor_obj.execute("select * from user_details")

            data = cursor_obj.execute('''SELECT * FROM user_details''')
    ...
    def dropTabel(self, table_name):
        ...
        if table_name == "user_details":
            conn = sqlite3.connect('database//users.db')
            cursor_obj = conn.cursor()
            cursor_obj.execute('''drop table if exists user_details''')
            conn.commit()
            conn.close()
            # print("entered droptable.")
        # self.logger.write_logs("Table Train dropped successfully.")
    ...
    def get_users_list(self):
        ...
        data = cursor_obj.execute('''SELECT DISTINCT user_name FROM user_details''')

        return data.description


    def delete_rows_per_user(self, user):

        conn = sqlite3.connect('database//predictionDatabase.db')
        cursor_obj = conn.cursor()
        cursor_obj.execute(f" DELETE FROM Predictions WHERE user_name='{user}'; ")
