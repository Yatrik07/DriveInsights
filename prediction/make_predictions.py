...
def classify_vehicle(img , shape = (400,400)):
    print(type(img) , img.shape)
    model = load_model(os.path.join('models' , 'vehicle_classification.h5'))
    img = resizeAndScale(img , image_shape=shape)
    img = np.expand_dims(img, 0)
    pred = model.predict( img )
    # map = {0: 'Auto', 1: 'bike', 2: 'car'}
    map = {0: 'Auto', 1: 'two-wheeler', 2: 'four-wheeler'}
    output = map[ np.argmax( pred ) ]
    return output
